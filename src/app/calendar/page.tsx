...
function CalendarGridSkeleton() {
  ...
  const fd = firstDay(today.getFullYear(), today.getMonth());
  const cells: (number | null)[] = [...Array(fd).fill(null), ...Array.from({ length: dim }, (_, i) => i + 1)];

  return (
    <SectionCard>
      <div className="flex items-center justify-between mb-4">
        <div className="bg-stone-100/50 rounded h-6 w-6" />
        <div className="bg-stone-100/50 rounded h-5 w-24" />
        <div className="bg-stone-100/50 rounded h-6 w-6" />
      </div>

      <div className="grid grid-cols-7 gap-1 mb-1">
        {DAYS.map((d) => (
          <div key={d} className="text-center text-[9px] font-semibold uppercase text-stone-500 py-1">{d}</div>
        ))}
      </div>

      <div className="grid grid-cols-7 gap-1">
        {cells.map((_, i) => (
          <div key={i} className="min-h-[44px] rounded-lg bg-stone-100/30" />
        ))}
      </div>
    </SectionCard>
  );
...
function CalendarNotesSkeleton() {
  return (
    <SectionCard title="Notes">
      <div className="space-y-2">
        <div className="bg-stone-100/30 rounded h-8 w-full" />
        <div className="bg-stone-100/30 rounded h-8 w-3/4" />
      </div>
    </SectionCard>
  );
}
    ...
    <SectionCard title="Scheduled Jobs">
      <div className="space-y-2">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="bg-stone-100/30 rounded h-8 w-full" />
        ))}
      </div>
    </SectionCard>
  );
...
function CalendarContent() {
  ...
  return (
    <>
                ...
                <button
                  ...
                  className={[
                    ...
                    sel              ? "border border-cyan-400/40 bg-cyan-500/15 text-cyan-100"             :
                                       "border border-stone-200/50 bg-stone-50/40 text-stone-600 hover:border-stone-300/60 hover:bg-stone-100/50",
                  ].join(" ")}
                >
                  {d}
                  {d && hasN && (
      ...
      {/* Selected day notes with Suspense */}
      <Suspense fallback={<CalendarNotesSkeleton />}>
        {selectedDay && (
          <SectionCard title={selectedDateStr ?? ""}>
            <div className="space-y-1 max-h-[130px] overflow-y-auto mb-3">
              {selectedNotes.length === 0 ? (
                <p className="text-[10px] text-stone-500 py-2">No notes for this day</p>
              ) : selectedNotes.map((n) => (
                <div key={String(n._id)} className="flex items-center gap-2 px-2.5 py-1.5 text-xs panel-soft">
                  <span className="flex-1 text-stone-700">{n.note}</span>
                  <button onClick={() => void deleteNote({ id: n._id })} className="text-stone-500 hover:text-rose-400 transition-colors text-sm leading-none">×</button>
                </div>
              ))}
            </div>
      ...
      {/* Scheduled Jobs with Suspense */}
      <Suspense fallback={<ScheduledJobsSkeleton />}>
        <SectionCard title="Scheduled Jobs">
          <div className="space-y-1 max-h-[220px] overflow-y-auto">
            {loadingJobs ? (
              <p className="text-xs text-stone-500 text-center py-3">Loading…</p>
            ) : jobs.length === 0 ? (
              <p className="text-xs text-stone-500 text-center py-3">No cron jobs</p>
            ) : (
              jobs.slice(0, 10).map((job, i) => (
                <div key={job.id ?? i} className="flex items-center gap-2.5 px-2.5 py-2 text-xs panel-soft">
                  <span className="text-stone-700 font-medium truncate flex-1">{job.name}</span>
